...
test.describe("InlineChangeEditor core API", () => {
  ...
  test("constructor adds paragraph to empty element", async ({ page }) => {
    ...
    const result = await page.evaluate(() => {
      ...
      return {
        ...
        rangeStart:
          changeEditor.env.selection.getRangeAt(0).startContainer.nodeName,
      };
    });
  ...
  test("getCleanContent strips inserts and deletes", async ({ page }) => {
    await page.setContent(
      '<div id="el"><p>test <span class="ins cts-1" cid="1">content</span> in paragraph one.</p><p>test <em><span class="del cts-2" cid="2">content <span class="ins" cid="3">in</span></span> paragraph</em> two.</p></div>',
    );
    const result = await page.evaluate(() => {
      const el = document.querySelector("#el");
      const changeEditor = getIce(el);
    ...
    expect(result.textContent).toBe(
      "test content in paragraph one.test  paragraph two.",
    );
  });

  test("acceptAll removes inserts and deletes", async ({ page }) => {
    await page.setContent(
      '<div id="el"><p>test <span class="ins" cid="1">content<span class="ins" cid="4"> in</span></span> paragraph one.</p><p>test <em><span class="del" cid="2">content <span class="ins" cid="3">in</span></span> paragraph</em> two.</p></div>',
    );
    await page.evaluate(() => {
      const el = document.querySelector("#el");
      const changeEditor = getIce(el);
    ...
    const insDelCount = await page.$$eval(
      "#el .ins, #el .del",
      (els) => els.length,
    );
  ...
  test("acceptChange accepts specific changes", async ({ page }) => {
    await page.setContent(
      '<div id="el"><p>test <span class="ins" cid="1">content<span class="ins" cid="4"> in</span></span> paragraph one.</p><p>test <em><span class="del" cid="2">content <span class="ins" cid="3">in</span></span><span class="del" cid="2">batch change cid</span> paragraph</em> two.</p></div>',
    );
    await page.evaluate(() => {
      const el = document.querySelector("#el");
      const changeEditor = getIce(el);
    ...
    const elText = await page.$eval("#el", (el) => el.textContent);
    const cidCount = await page.$$eval(
      '#el [cid="4"], #el [cid="2"]',
      (els) => els.length,
    );
  ...
  test("rejectChange rejects specific changes", async ({ page }) => {
    ...
    expect(cidCount).toBe(0);
    expect(elText).toBe(
      "test content paragraph one.test content in paragraph two.",
    );
  });

  test("placeholdDeletes adds placeholders for deletes", async ({ page }) => {
    await page.setContent(
      '<div id="el"><p>test <span class="del cts-1" cid="1">content</span> in paragraph one.</p><p>test <em><span class="del cts-2" cid="2">content <span class="ins" cid="3">in</span></span> paragraph</em> two.</p></div>',
    );
    const result = await page.evaluate(() => {
      const el = document.querySelector("#el");
      const changeEditor = getIce(el);
      ...
      return {
        ...
        p0alloc: el
          ...
          .querySelector("tempdel")
          ?.getAttribute("data-allocation"),
  ...
  test("revertDeletePlaceholders restores original HTML", async ({ page }) => {
    const html =
      '<p>test <span class="del cts-1" cid="1">content</span> in paragraph one.</p><p>test <em><span class="del cts-2" cid="2">content <span class="ins" cid="3">in</span></span> paragraph</em> two.</p>';
