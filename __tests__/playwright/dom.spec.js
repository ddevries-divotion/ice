...
test.describe("dom.js utility functions", () => {
  ...
  test("dom.noInclusionInherits copies prototype properties", async ({
    page,
  }) => {
    const result = await page.evaluate(() => {
      const dom = window.ice.dom;
  ...
  test("dom.isset returns true for set values", async ({ page }) => {
    ...
    expect(result.undefinedVal).toBe(false);
  });
  ...
  test("dom.arrayDiff returns difference between arrays", async ({ page }) => {
    ...
    expect(result.bothArrays).toEqual([1, 4]);
  });
...
test.describe("dom.js date and timestamp functions", () => {
  test("dom.date formats a timestamp", async ({ page }) => {
    const result = await page.evaluate(() =>
      window.ice.dom.date("Y-m-d", 946684800000),
    ); // 2000-01-01
  ...
  test("dom.date formats ISO 8601 string", async ({ page }) => {
    const result = await page.evaluate(() =>
      window.ice.dom.date("Y-m-d", null, "2000-01-01T00:00:00Z"),
    );
    expect(result).toBe("2000-01-01");
  });
  ...
  test("dom.tsIso8601ToTimestamp parses ISO 8601", async ({ page }) => {
    const ts = await page.evaluate(() =>
      window.ice.dom.tsIso8601ToTimestamp("2000-01-01T00:00:00Z"),
    );
    // Should be close to 946684800000 (Jan 1, 2000 UTC)
    expect(Math.abs(ts - 946684800000)).toBeLessThan(1000); // allow 1s drift
  });

  test("dom.tsIso8601ToTimestamp returns null for invalid", async ({
    page,
  }) => {
    const ts = await page.evaluate(() =>
      window.ice.dom.tsIso8601ToTimestamp("not-a-date"),
    );
    expect(ts).toBeNull();
  });
});
...
test.describe("jQuery-based dom.js functions", () => {
  test("`getClass` returns elements with the given class", async ({ page }) => {
    ...
    const result = await page.evaluate(
      () => window.ice.dom.getClass("foo").length,
    );
  ...
  test("`empty` removes all children", async ({ page }) => {
    ...
    await page.evaluate(() =>
      window.ice.dom.empty(document.getElementById("test")),
    );
  ...
  test("`remove` deletes the element", async ({ page }) => {
    ...
    await page.evaluate(() =>
      window.ice.dom.remove(document.getElementById("test")),
    );
  ...
  test("`insertBefore` inserts before the reference node", async ({ page }) => {
    ...
    const tag = await page.evaluate(
      () => document.getElementById("ref").previousSibling.tagName,
    );
    expect(tag).toBe("B");
  });
  ...
  test("`insertAfter` inserts after the reference node", async ({ page }) => {
    ...
    const tag = await page.evaluate(
      () => document.getElementById("ref").nextSibling.tagName,
    );
    expect(tag).toBe("B");
  });
  ...
  test("`contents` returns child nodes as array", async ({ page }) => {
    ...
    const result = await page.evaluate(
      () => window.ice.dom.contents(document.getElementById("test")).length,
    );
    expect(result).toBe(2);
  });
  ...
  test("`getNodeTextContent` returns text content", async ({ page }) => {
    ...
    const text = await page.evaluate(() =>
      window.ice.dom.getNodeTextContent(document.getElementById("test")),
    );
    expect(text).toContain("hello");
  });

  // Parallel tests for native browser API equivalents
  test("`getClass` (native) returns elements with the given class", async ({
    page,
  }) => {
    await page.setContent('<div class="foo"></div><div class="foo"></div>');
    const result = await page.evaluate(
      () => Array.from(document.getElementsByClassName("foo")).length,
    );
    expect(result).toBe(2);
  });
  ...
  test("`insertBefore` (native) inserts before the reference node", async ({
    page,
  }) => {
    await page.setContent('<div id="parent"><span id="ref"></span></div>');
    await page.evaluate(() => {
  ...
  test("`insertAfter` (native) inserts after the reference node", async ({
    page,
  }) => {
    await page.setContent('<div id="parent"><span id="ref"></span></div>');
    await page.evaluate(() => {
  ...
  test("`contents` (native) returns child nodes as array", async ({ page }) => {
    await page.setContent('<div id="test"><span></span><b></b></div>');
    const result = await page.evaluate(
      () => Array.from(document.getElementById("test").childNodes).length,
    );
    expect(result).toBe(2);
  });

  test("`getNodeTextContent` (native) returns text content", async ({
    page,
  }) => {
    await page.setContent('<div id="test">hello <b>world</b></div>');
    const text = await page.evaluate(
