...
(() => {
	tinymce.PluginManager.add('ice', (editor, pluginUrl) => {
		...
		function isInsideChangeTag(node) {
			return editor.dom.getParent(node, `.${config.insertClass},.${config.deleteClass}`);
		}

		function cleanup() {
			const emptyNodes = editor.dom.select(`.${config.insertClass}:empty, .${config.deleteClass}:empty`);
		...
		editor.on('init', () => {
			...
			const cssPath = config.css.includes('://') ? config.css : `${pluginUrl}/${config.css}`;
			editor.dom.loadCSS(cssPath);

			loadScript(`${pluginUrl}/js/ice.min.js?version=${config.scriptVersion}`, () => {
		...
		registerCommand('icecleanbody', (el) => {
			const body = changeEditor.getCleanContent(el || editor.getContent(), config.afterClean, config.beforeClean);
			return body;
		});
		...
		registerCommand('ice_disable', () => {
			...
			['acceptAllButton', 'rejectAllButton', 'acceptButton', 'rejectButton'].forEach(btn => {
		...
		registerCommand('ice_togglechanges', () => {
			editor.execCommand(changeEditor.isTracking ? 'ice_disable' : 'ice_enable');
		});
		...
		registerCommand('ice_smartquotes', () => {
			changeEditor.pluginsManager.plugins.IceSmartQuotesPlugin.convert(editor.getBody());
			editor.windowManager.alert('Regular quotes have been converted into smart quotes.');
		});
